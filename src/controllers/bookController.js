...
const createBook = async function (req, res) {
    ...
    if (!req.body.author) return res.send({ msg: "authorId is required!" })

    else if (!validObj) return res.send({ msg: "Please enter vallid AuthorId!" })

    else if (!req.body.publisher) return res.send({ msg: "publisherId is required!" })

    else if (!validObjPub) return res.send({ msg: "Please enter vallid PublisherId!" })
    else {
